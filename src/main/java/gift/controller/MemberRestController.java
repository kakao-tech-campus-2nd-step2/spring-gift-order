...
public class MemberRestController {
    ...
    @PostMapping("/login")
    public ResponseEntity<JwtTokenResponse> giveAccessToken(@RequestBody MemberRequest request) {
        memberService.checkMemberExistsByEmailAndPassword(request.email(),request.password());
        String token = jwtUtil.generateToken(request.email());
        HttpHeaders headers = new HttpHeaders();
        headers.set("Authorization", "Bearer " + token);

        return ResponseEntity.status(HttpStatus.OK).headers(headers).body(new JwtTokenResponse(token));
    }
}
