...
- 내 애플리케이션 > 앱 설정 > 앱 키로 이동하여 REST API 키를 복사한다.
  https://kauth.kakao.com/oauth/authorize?scope=talk_message&response_type=code&redirect_uri=http://localhost:8080&client_id={REST_API_KEY}에
- 접속하여 카카오톡 메시지 전송에 동의한다. http://localhost:8080/?code={AUTHORIZATION_CODE}에서 인가 코드를 추출한다.
